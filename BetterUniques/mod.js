...
uniqueItems.rows.forEach((item) => {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    if (propN != null
      && BOOST_PROP_ARRAY.includes(propN)) {
      switch (propN) {
        case 'light':
          item[`min` + i] = Math.floor(minN * 1.5);
          item[`max` + i] = Math.floor(maxN * 1.5);
          break;
        case 'ac':
          item[`min` + i] = Math.floor(minN * 1.5);
          item[`max` + i] = Math.floor(maxN * 1.5);
...
uniqueItems.rows.push({
  ...
  prop3: 'crush', min3: 10, max3: 15, prop4: 'ease', min4: 20, max4: 20,
});
...
uniqueItems.rows.push({
  ...
  prop3: 'swing2', min3: 20, max3: 30, prop4: 'ease', min4: -20, max4: -20, par4: null,
});
uniqueItems.rows.push({
  ...rainbowFacet, index: "Rainbow Stone", '*ID': (itemID = itemID + 1),
  carry1: 0, lvl: 70, 'lvl req': 50, '*eol\r': '0',
  prop1: 'res-all', min1: 10, max1: 20, prop2: 'red-mag', min2: 6, max2: 12,
  prop3: 'mana-kill', min3: 4, max3: 8, prop4: 'ease', min4: -20, max4: -20,
});
